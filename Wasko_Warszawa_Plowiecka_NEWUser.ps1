...
$pathOU = "OU=Zarzad, OU=RN, OU=WASKO, DC=wasko, DC=pl"
...
[string]$AccountExpirationDate = Read-Host "Wygaśnięcie konta: N [Never] lub [dd-mm-rr]"
$Add0=Read-Host "Dodać do grupy - HCPAW-RegularUsers ? T [Tak]"
$Add1=Read-Host "Dodać do grupy - O365SyncUser ? T [Tak]"
#$Add2=Read-Host "Dodać do grupy:  Print_Mentalistka_kolor: Wybierz T [tak] lub ENTER [dalej]"
...
if ( $next1 -eq "t" ) {  C:\Scripts\ADDNewUSER.ps1 }
 ...
 New-ADUser `
            ...
            -Path $mainpath `
            -EmailAddress $SamAccountUser_email@wasko.pl `
            -Title $Title `
            -Manager $Manager[3] `
            -ChangePasswordAtLogon $true `
            -StreetAddress "Płowiecka 105/107" `
            -City "Warszawa" `
            -Company "WASKO S.A." `
            -Country "pl" `
            -PostalCode "04-501" `
    ...
    Write-Output "E-mail: $SamAccountUser_email@wasko.pl" | Out-File  C:\NewUsersList.txt -append
...
$path_notepad = "C:\$Firstname $Lastname - Poświadczenia.txt"
    ...
    Write-Output "E-mail: $SamAccountUser_email@$server" | Out-File  "$path_notepad" -Append
