...
$pathOU = "OU=ente, DC=ente, DC=local"
...
$token = $baza.Item([int]$it)
$token1 = ($token.Split(",-")).ForEach({ "$_," })
if ($token1[6]) {
    $alltoken = $token1[0]+$token1[2]+$token1[4]+$token1[5]+$token1[6].Trimend(",")
    $mainpath = $alltoken.Replace(' ','')}
elseif ($token1[4]) {
    $alltoken1 = $token1[0]+$token1[2]+$token1[3]+$token1[4].Trimend(",")
    $mainpath = $alltoken1.Replace(' ','')}
elseif ($token1[2]) {
    $alltoken1 = $token1[0]+$token1[1]+$token1[2].Trimend(",")
    $mainpath = $alltoken1.Replace(' ','')}
...
#$Add0=Read-Host "Dodać do grupy:  VPN_Korona_NOW: Wybierz T [tak] lub ENTER [dalej]"
$Firstname_user = $Firstname[0]
$Lastname_user = $Lastname[0..6] -join ''
#$Lastname_user1 = $Lastname[0..25] -join ''
...
if ( $next1 -eq "t" ) {  C:\Scripts\ADDNewUSER.ps1 }
  ...
  New-ADUser `
            ...
            -UserPrincipalName $SamAccountUser@ente.local `
            ...
            -Path $mainpath `
            -EmailAddress $SamAccountUser_email@ente.com.pl `
            -Title $Title `
            -Manager $Manager[3] `
    ...
    Write-Output ""
    Write-host -ForegroundColor Yellow "Dodano użytkownika '$SamAccountUser'"
    ...
    Write-Output "Użytkownik: ente.local\$SamAccountUser" | Out-File  C:\NewUsersList.txt -append
    Write-Output "E-mail: $SamAccountUser_email@ente.com.pl" | Out-File  C:\NewUsersList.txt -append
...
$Password = "$Pass"
$mx = "$SamAccountUser_email@owa.ente.com.pl"
...
$path_notepad = "C:\$Firstname $Lastname - Poświadczenia.txt"
    ...
    Write-Output "Użytkownik: $server\$SamAccountUser" | Out-File  "$path_notepad" -Append
    Write-Output "E-mail: $SamAccountUser_email@ente.com.pl" | Out-File  "$path_notepad" -Append
    Write-Output "Hasło do zmiany: $Pass" | Out-File  "$path_notepad" -Append


#variable for it account to send email like CC
$username
$itusername = ($username.split("\")[1])
