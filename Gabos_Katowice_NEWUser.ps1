...
$Credentials = Get-StoredCredential -Target win.gabos.pl
$Username = $Credentials.UserName
$Password = $Credentials.Password
$Server = "win.gabos.pl"
$Credentials = New-Object System.Management.Automation.PSCredential $Username,$Password

$pathOU = "OU=GABOS, DC=win, DC=gabos, DC=pl"
...
$Add0=Read-Host "Dodać do grupy - ADAzureSync ? T [Tak]"
$Add1=Read-Host "Dodać do grupy - HCPAW Users ? T [Tak]"
...
if ( $next1 -eq "t" ) {  C:\Scripts\ADDNewUSER.ps1 }
 ...
 New-ADUser `
            ...
            -UserPrincipalName $SamAccountUser@gabos.pl `
            ...
            -Path $mainpath `
            -EmailAddress $SamAccountUser_email@gabos.pl `
            -Title $Title `
            -Manager $Manager[3] `
     ...
     if ($AccountExpirationDate -eq "n") {Clear-ADAccountExpiration -Identity $SamAccountUser -Credential $Credentials -Server $Server}
        ...
        if ($Add0 -eq "t")
       {
       Add-ADGroupMember -Identity "ADAzureSync" -Members $SamAccountUser -Credential $Credentials -Server $Server
       }
        if ($Add1 -eq "t")
       {
       Add-ADGroupMember -Identity "HCPAW Users" -Members $SamAccountUser -Credential $Credentials -Server $Server
       }
    ...
    Write-Output "Użytkownik: WINGABOS\$SamAccountUser" | Out-File  C:\NewUsersList.txt -append
    Write-Output "E-mail: $SamAccountUser_email@gabos.pl" | Out-File  C:\NewUsersList.txt -append
...
$Cred = Get-StoredCredential -Target mxgabos
...
$Password = "$Pass"
$mx = "$SamAccountUser_email@owa.gabos.pl"
...
$path_notepad = "C:\$Firstname $Lastname - Poświadczenia.txt"
    ...
    Write-Output "Użytkownik: WINGABOS\$SamAccountUser" | Out-File  "$path_notepad" -Append
    Write-Output "E-mail: $SamAccountUser_email@gabos.pl" | Out-File  "$path_notepad" -Append
