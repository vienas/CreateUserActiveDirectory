...
$pathOU = "OU=D, OU=COIG SA, DC=win, DC=coig, DC=com"
...
$AccountExpirationDate = Read-Host "Wygaśnięcie konta: N [Never] lub [dd-mm-rr]"
$Add0=Read-Host "Dodać do grupy - AzureADUsers ? T [Tak]"
$Add1=Read-Host "Dodać do grupy - VPN-COIG ? T [Tak]"
$Firstname_user = $Firstname[0]
$SamAccountUser = ($Firstname_user + $Lastname).ToLower()
$SamAccountUser_email = ($Firstname + "." + $Lastname).ToLower()
...
do {$i++; $star="*"; $searchMG2 = "";
        $searchMG = $(Write-Host "MENADŻER - SZUKAJ: " -ForegroundColor Cyan -NoNewLine; Read-Host);
                ...
                if ([string]::IsNullOrWhiteSpace($searchMG)) { Write-Warning "Błędne dane wejściowe" }
                    ...
                    else {$pathMG = @(Get-ADUser -Filter { Displayname -like $searchMG2 } -Properties * -Credential $Credentials -Server $Server -SearchBase $pathOU  | Select-Object Surname, Givename, EmailAddress, SamAccountName, Displayname, Department)
...
foreach ($_ in $pathMG ) { $numermg++
    Write-Host -ForegroundColor DarkCyan $numermg. $_.Displayname "," $_.Department;
...
if ([string]::IsNullOrWhiteSpace($Title) -or ([string]::IsNullOrWhiteSpace($Department))) {
    Write-Host -ForegroundColor Red "Pole Dział nie może być puste"
    $licz++}
...
if ( $next1 -eq "t" ) {  C:\Scripts\ADDNewUSER.ps1 }
 ...
 New-ADUser `
            ...
            -UserPrincipalName $SamAccountUser@coig.pl `
            ...
            -Path $mainpath `
            -EmailAddress $SamAccountUser_email@coig.pl `
            -Manager $Manager[3] `
    ...
    Write-Output "Użytkownik: WINCOIG\$SamAccountUser" | Out-File  C:\NewUsersList.txt -append
    Write-Output "E-mail: $SamAccountUser_email@coig.pl" | Out-File  C:\NewUsersList.txt -append
...
$Cred = Get-StoredCredential -Target mxcoig
...
$path_notepad = "C:\$Firstname $Lastname - Poświadczenia.txt"
    ...
    Write-Output "Użytkownik: WINCOIG\$SamAccountUser" | Out-File  "$path_notepad" -Append
    Write-Output "E-mail: $SamAccountUser_email@coig.pl" | Out-File  "$path_notepad" -Append
