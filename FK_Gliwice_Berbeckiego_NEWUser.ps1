...
$pathOU = "OU=W4B, OU=Wspolpracownicy, OU=WASKO, DC=fk, DC=wasko, DC=pl"
...
$Add0=Read-Host "Dodać do grupy - HCP ? T [Tak]"
$Add1=Read-Host "Dodać do grupy - VPN_FK ? T [Tak]"
$Add2=Read-Host "Dodać do grupy - Księgowość ? T [Tak]"
...
if ( $next1 -eq "t" ) {  C:\Scripts\ADDNewUSER.ps1 }
 ...
 New-ADUser `
            ...
            -UserPrincipalName $SamAccountUser@fk.wasko.pl `
            ...
            -Path $mainpath `
            -EmailAddress $SamAccountUser_email@wasko4b.pl `
            -Title $Title `
            -Manager $Manager[3] `
            -ChangePasswordAtLogon $true `
            -StreetAddress "Berbeckiego 6" `
            -City "Wasko - Centrala Gliwice" `
            -Company "WASKO4BUSINESS SP. Z O.O." `
            -Country "pl" `
            -PostalCode "44100" `
            -Office "Pracownicy Wasko" `
    ...
    Write-Output "Użytkownik: FK\$SamAccountUser" | Out-File  C:\NewUsersList.txt -append
    Write-Output "E-mail: $SamAccountUser_email@wasko4b.pl" | Out-File  C:\NewUsersList.txt -append
...
$Cred = Get-StoredCredential -Target mxwasko4b
...
$path_notepad = "C:\$Firstname $Lastname - Poświadczenia.txt"
    ...
    Write-Output "Użytkownik: FK\$SamAccountUser" | Out-File  "$path_notepad" -Append
    Write-Output "E-mail: $SamAccountUser_email@wasko4b.pl" | Out-File  "$path_notepad" -Append
