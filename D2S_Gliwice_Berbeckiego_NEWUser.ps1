...
$pathOU = "OU=D2S, OU=Wspolpracownicy, OU=WASKO, DC=wasko, DC=pl"
...
$Add0=Read-Host "Dodać do grupy - D2S Users ? T [Tak]"
$Add1=Read-Host "Dodać do grupy - O365SyncUser ? T [Tak]"
$Add2=Read-Host "Dodać do grupy - HCPAW-RegularUsers ? T [Tak]"
$Add3=Read-Host "Dodać do grupy - D2S_Dzial_Wsparcia_Projektów ? T [Tak]"
...
if ( $next1 -eq "t" ) {  C:\Scripts\ADDNewUSER.ps1 }
 ...
 New-ADUser `
            ...
            -Description "D2S, $Firstname $Lastname" `
            -Department "D2S" `
            -Path $mainpath `
            -EmailAddress $SamAccountUser_email@de2es.pl `
            -Title $Title `
            -Manager $Manager[3] `
            -ChangePasswordAtLogon $true `
            -StreetAddress "Berbeckiego 6" `
            -City "Gliwice" `
            -Company "D2S sp. z o.o." `
            -Country "pl" `
            -PostalCode "44-100" `
     ...
     if ($AccountExpirationDate -eq "n") {Clear-ADAccountExpiration -Identity $SamAccountUser -Credential $Credentials -Server $Server}
       ...
       Add-ADGroupMember -Identity "HCPAW-RegularUsers" -Members $SamAccountUser -Credential $Credentials -Server $Server
        ...
        if ($Add3 -eq "t")
       {
       Add-ADGroupMember -Identity "D2S_Dzial_Wsparcia_Projektówr" -Members $SamAccountUser -Credential $Credentials -Server $Server
       }
    ...
    Write-Output "E-mail: $SamAccountUser_email@de2es.pl" | Out-File  C:\NewUsersList.txt -append
...
$Cred = Get-StoredCredential -Target mxde2es
...
$path_notepad = "C:\$Firstname $Lastname - Poświadczenia.txt"
    ...
    Write-Output "E-mail: $SamAccountUser_email@de2es.pl" | Out-File  "$path_notepad" -Append
