cls
Write-Output "FONON GLIWICE BERBECKIEGO"
...
if ([string]::IsNullOrWhiteSpace($Firstname_manager) -or ([string]::IsNullOrWhiteSpace($Lastname_manager))) {
    Write-Host -ForegroundColor Red "Pola Imię menadżera, Nazwisko menadżera nie mogą być puste"
    }
if ([string]::IsNullOrWhiteSpace($Title) -or ([string]::IsNullOrWhiteSpace($Department))) {
 ...
 New-ADUser `
        ...
        if ($Add0 -eq "t")
       {
       Add-ADGroupMember -Identity "HCPAW-RegularUsers" -Members $SamAccountUser
       }
        if ($Add1 -eq "t")
       {
       Add-ADGroupMember -Identity "O365SyncUser" -Members $SamAccountUser
       }
        if ($Add2 -eq "t")
       {
       Add-ADGroupMember -Identity "ALFA_ProjektyFONON_100016_NOKIA-RW" -Members $SamAccountUser
       }
        if ($Add3 -eq "t")
       {
       Add-ADGroupMember -Identity "ALFA_ProjektyFONON_100026_GSMR-RW" -Members $SamAccountUser
       }
        if ($Add4 -eq "t")
       {
       Add-ADGroupMember -Identity "ALFA_ProjektyFONON_Pomoce_naukowe-RW" -Members $SamAccountUser
       }
        if ($Add5 -eq "t")
       {
       Add-ADGroupMember -Identity "ALFA_ProjektyFONON_Niemcy_projektowanie-RW" -Members $SamAccountUser
       }
